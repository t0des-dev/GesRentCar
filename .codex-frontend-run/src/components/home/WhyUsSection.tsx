...
export default function WhyUsSection({ agency, content, aboutText }: { agency: any; content: any; aboutText: string }) {
  ...
  const features = (agency.features_config && (agency.features_config as any[]).length > 0)
    ? (agency.features_config as any[]).map((f: any) => {
        const iconMap: Record<string, any> = { HeadphonesIcon, ShieldCheck, Star, Car, LayoutDashboard, Settings, Users };
        return { icon: iconMap[f.icon] || Crown, title: f.title, desc: f.desc };
      })
    : defaultFeatures;
    ...
    <section className="py-24 bg-slate-50">
      <div className="container mx-auto px-6">
        <div className="max-w-2xl mb-16">
          ...
          >
            {content.subtitle || aboutText || t("features_subtitle")}
          </motion.p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {features.map(({ icon: Icon, title, desc }, idx) => (
            ...
            >
              <div className="w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 mb-6 group-hover:bg-primary/5 group-hover:text-primary transition-all duration-500">
