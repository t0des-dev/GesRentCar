      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-col gap-3">
          {TYPES.map((t) => (
            ...
            >
              <span>{getCategoryLabel(t).toUpperCase()}</span>
              <div className={cn(
                "w-2 h-2 rounded-full transition-all",
                filters.type === t ? "bg-white scale-125" : "bg-gold/40 group-hover:bg-gold/60"
              )} />
            </motion.button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {TRANSMISSIONS.map((tr) => (
            <motion.button
              ...
              className={cn(
                "flex items-center justify-center px-4 py-3 rounded-lg text-[10px] font-bold transition-all border-2 uppercase tracking-widest",
        ...
        <div className="flex flex-wrap gap-3">
          {SEATS.map((s) => (
            <motion.button
              ...
              className={cn(
                "w-14 h-14 flex items-center justify-center rounded-lg text-xs font-bold border-2 transition-all",
                filters.seats === s
                  ? "bg-primary text-white border-primary shadow-lg shadow-primary/30"
                  : "bg-surface-1 border-border text-ink-2 hover:border-gold/40 hover:bg-surface-2"
              )}
