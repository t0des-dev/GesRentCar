...
import type { MapLocation } from "@/types/storefront";
...
export default function ExperienceMap({ content = {} }: { content?: { badge?: string; slogan?: string; locations?: MapLocation[]; stats?: { community_value?: string; fleet_value?: string } } }) {
  const locations = (content?.locations && content.locations.length > 0) ? content.locations : DEFAULT_LOCATIONS;
        ...
        <div className="flex flex-col lg:flex-row items-center gap-16">
          ...
          <div className="flex-1 space-y-8">
            <div className="inline-flex items-center gap-3 bg-slate-900 text-white px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.3em]">
              ...
              {content?.badge || "Présence Nationale"}
            </div>
            ...
            <p className="text-lg text-slate-500 leading-relaxed max-w-xl">
              {content?.slogan || "Une sélection de prestige pour ceux qui ne font aucun compromis sur l'excellence."}
            </p>
            ...
            <div className="grid grid-cols-2 gap-6 pt-4">
              <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                ...
                <p className="text-3xl font-black text-slate-900">{stats.community_value || "400+"}</p>
              ...
              <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                ...
                <p className="text-3xl font-black text-slate-900">{stats.fleet_value || "80+"}</p>
          ...
          <div className="flex-1 w-full relative">
            <div className="relative bg-slate-900 rounded-[48px] p-8 shadow-2xl shadow-slate-900/40 overflow-hidden aspect-[4/5] md:aspect-square flex items-center justify-center border border-white/10">
              ...
              {locations.map((exp: MapLocation, i: number) => (
                <div
                  key={i}
