                ...
                <div className="flex flex-col gap-3">
                   ...
                   <div className="flex flex-col items-center gap-4 p-6 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                      <div className="w-full aspect-video rounded-xl overflow-hidden border border-slate-200 bg-white relative group">
                        <img
                          src={vehicle.new_image ? URL.createObjectURL(vehicle.new_image) : (vehicle.image_url ? `${(process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000/api").replace("/api", "")}${vehicle.image_url}` : 'https://placehold.co/800x450?text=Aucune+Image')}
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {vehicle?.photos?.map((url: string, i: number) => (
                      <div key={i} className="relative aspect-square rounded-xl overflow-hidden border border-slate-200 group">
                        <img src={`${(process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000/api").replace("/api", "")}${url}`} alt="Gallery" className="w-full h-full object-cover" />
                        <button
                          type="button"
