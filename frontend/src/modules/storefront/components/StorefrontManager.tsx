    ...
    if (currentAgency.agency_name && !justSavedRef.current) {
      setForm(prev => {
        const merged = {
          ...
          header_config: {
            ...prev.header_config,
            ...currentAgency.header_config,
            menu_links: currentAgency.header_config?.menu_links || prev.header_config.menu_links,
          },
        ...
        if (missing.length > 0) {
          ...
          for (const sec of missing) {
            ...
            const prevInDefault = defIdx > 0 ? defaultOrder[defIdx - 1].id : null;
            const insertAfterIdx = prevInDefault ? order.findIndex((s: any) => s.id === prevInDefault) : -1;
            if (insertAfterIdx >= 0) {
              order.splice(insertAfterIdx + 1, 0, sec);
            } else {
              order.push(sec);
            }
          }
          merged.sections_order = order;
        }
