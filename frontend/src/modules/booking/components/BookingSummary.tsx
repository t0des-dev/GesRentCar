...
import { Pencil, Star, Check, Phone, Lock, Plane, Headphones, Loader2, Info } from "lucide-react";
...
export default function BookingSummary({
  booking, days, vehicle, vehicleLoading,
  currentStep = 0, onEditVehicle,
}: BookingSummaryProps) {
  useCurrency();
  ...
  const formatDate = (d?: string) => {
    if (!d) return "—";
    return new Date(d).toLocaleDateString(lang === "fr" ? "fr-FR" : "en-US", { day: "2-digit", month: "short", year: "numeric" });
  };
    ...
    <aside className="w-full sticky top-32 space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-lg shadow-black/5 overflow-hidden">
        <div className="p-6 space-y-5">
          {vehicleLoading ? (
            <div className="flex items-center gap-3 text-gray-400 py-8 justify-center">
              ...
              <p className="text-sm font-medium">{t("identity_loading")}</p>
            </div>
          ) : vehicle ? (
            <>
              {/* Vehicle Header */}
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {vehicle.price?.toLocaleString()} MAD {t("quickview_per_day")}
                  </p>
                  {days > 0 && (
                    <p className="text-[11px] text-gray-500 mt-1">
                      {days} {days > 1 ? t("quickview_days") : t("quickview_day")} · {formatDate(booking.startDate)} → {formatDate(booking.endDate)}
                    </p>
                  )}
                </div>
              ...
              {days > 0 && (
                <div className="space-y-3 pt-4 border-t border-gray-100">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-semibold text-gray-900">{insurance > 0 ? `${insurance} MAD` : t("confirm_insurance_not")}</span>
                  </div>
                  <div className="flex justify-between items-center">
              ...
              {days > 0 && (
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-sm font-bold text-gray-900">{t("confirm_deposit")} (30%)</span>
                    <span className="text-lg font-black text-gray-900">{fmt(depositAmount)} MAD</span>
                  </div>
                  <p className="text-[11px] text-gray-400">
                    {t("confirm_remaining")} {fmt(totalCalc - depositAmount)} MAD {t("confirm_remaining_note")}
                  </p>
                </div>
              )}
            </>
            ...
            <div className="flex flex-col items-center gap-2 text-gray-400 py-8 border-2 border-dashed border-gray-200 rounded-2xl">
              <Info size={20} />
              <p className="text-sm font-medium">{t("fleet_empty_title")}</p>
            </div>
          )}
        </div>
        ...
        <div className="px-6 pb-6 space-y-3">
          ...
          {[
            { icon: Lock, label: t("confirm_incl_payment") },
            ...
            { icon: Star, label: t("feat_fleet_title") },
          ].map((item, i) => (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
        <h4 className="text-sm font-bold text-gray-900">{t("confirm_help_title")}</h4>
