...
export default function ConciergeBanner({ content }: ConciergeBannerProps) {
  const openConcierge = () => {
          ...
          <div className="flex items-center gap-8 relative z-10 flex-shrink-0">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-gold">{content?.badge || "Assistance Personnalisée"}</span>
              </div>
              
              <h3 className="display-md text-white leading-tight">
                {content?.title || <>Besoin d'aide pour <span className="text-gold">choisir</span> ?</>}
              </h3>
              
              <p className="body-feature text-ink-2 max-w-xl">
                {content?.description || "Laissez notre Concierge IA vous guider vers le véhicule parfait selon votre occasion et votre style."}
              </p>
            </div>
          </div>
