  ...
  return (
    <main className="min-h-screen bg-slate-50">
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-3 bg-white/5 backdrop-blur-md border border-white/10 px-6 py-2 rounded-full mb-8">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.4em] opacity-80">L'Excellence Vectoria</span>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
            ...
            >
              ...
              <motion.div 
                ...
                className="absolute -bottom-10 -right-10 bg-primary text-white p-10 rounded-[40px] shadow-2xl z-20 hidden md:block"
              >
                <p className="text-5xl font-black mb-1">5.0</p>
                <div className="flex gap-1 mb-4">
                  {[1,2,3,4,5].map(i => <Sparkles key={i} size={12} className="fill-white" />)}
                </div>
                <p className="text-[10px] font-black uppercase tracking-widest opacity-70">Note Client Moyenne</p>
              </motion.div>
            </motion.div>
            ...
            >
              <div>
                <p className="text-primary font-black text-xs uppercase tracking-[0.3em] mb-4">{t("about_story_label")}</p>
              ...
              <div className="grid grid-cols-2 gap-8 pt-8">
                <div className="p-8 bg-white rounded-[32px] shadow-sm border border-slate-100 group hover:border-primary/20 transition-all">
                  <h3 className="text-3xl font-black text-slate-900 mb-2 group-hover:text-primary transition-colors">24/7</h3>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t("stat_support")}</p>
                </div>
                <div className="p-8 bg-white rounded-[32px] shadow-sm border border-slate-100 group hover:border-primary/20 transition-all">
                  <h3 className="text-3xl font-black text-slate-900 mb-2 group-hover:text-primary transition-colors">100%</h3>
      ...
      <section className="py-32 bg-slate-900 text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="text-center mb-24">
            <h2 className="text-4xl md:text-6xl font-black tracking-tighter mb-6">{t("about_values_title")}</h2>
      ...
      <section className="py-32">
        <div className="container mx-auto px-6">
          <div className="bg-white rounded-[64px] p-12 md:p-24 shadow-2xl shadow-slate-200 border border-slate-100 flex flex-col lg:flex-row items-center gap-16 overflow-hidden relative">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-4">
                {["Luxe", "Sûreté", "Prestige", "Innovation"].map(tag => (
                  <span key={tag} className="px-6 py-2 rounded-full bg-slate-100 text-slate-600 text-xs font-black uppercase tracking-widest">
                    #{tag}
                  </span>
                ))}
              </div>
            ...
            <div className="w-full lg:w-[450px] aspect-square bg-slate-50 rounded-[48px] flex items-center justify-center relative group">
              <div className="absolute inset-0 bg-primary/10 scale-0 group-hover:scale-100 rounded-[48px] transition-transform duration-700 opacity-20" />
              <Crown size={120} className="text-primary/20 group-hover:text-primary/40 transition-colors duration-500" />
              <div className="absolute bottom-10 left-10 right-10 p-6 glass-card border border-white/20 text-center">
                 <p className="font-black text-slate-900 uppercase tracking-tighter italic">L'Expérience Vectoria</p>
              </div>
            </div>
          </div>
        </div>
