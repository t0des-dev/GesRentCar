...
{
    /**
     * Return current database counts for demo stats.
     */
    ...
    public function populate(): JsonResponse
    {
        try {
            DB::transaction(function () {
                ...
                if (count($createdClients) > 0 && count($createdVehicles) > 1) {
                    $reservationData = [
                        [
                ...
                if (count($createdVehicles) > 4) {
                    Maintenance::firstOrCreate(
                        ['vehicle_id' => $createdVehicles[4]->id, 'type' => 'oil'],
                        [
            ...
            return response()->json(['message' => 'Flotte et données démo initialisées avec succès !']);
    ...
    public function clear(): JsonResponse
    {
        try {
            DB::transaction(function () {
                ...
                // Keep admin & agent users, remove client role users
