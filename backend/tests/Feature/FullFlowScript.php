...
if ($login['status'] != 200) {
    exit('Login failed: '.json_encode($login)."\n");
}
...
if ($scan['status'] != 200) {
    exit('OCR failed: '.json_encode($scan)."\n");
}
...
if ($clientSave['status'] != 201 && $clientSave['status'] != 200) {
    exit('Client save failed: '.json_encode($clientSave)."\n");
}
...
if ($reservation['status'] != 201 && $reservation['status'] != 200) {
    exit('Reservation failed: '.json_encode($reservation)."\n");
}
...
if ($contract['status'] != 200) {
    exit('Contract failed: '.json_encode($contract)."\n");
}
